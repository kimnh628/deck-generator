...
async function createIconCard(parent, data, opts) {
    ...
    const card = createCard(parent, `IconCard — ${data.title}`, Object.assign(Object.assign({}, opts), { bg: isDark ? C.g800 : C.white, shadow: !isDark }));
...
async function createGanttCard(parent, data, opts) {
    const card = createCard(parent, `Gantt — ${data.title}`, Object.assign(Object.assign({}, opts), { shadow: true }));
    const pad = S.padCard;
    const titleNode = await createText(card, {
        text: data.title, x: pad, y: pad, w: opts.w - pad * 2,
        size: 36, weight: 700, lh: 1.45, ls: -0.02, color: C.black,
...
async function createDataTableSlide(data) {
    ...
    const colWidths = data.columnWidths
...
async function createImagePlaceholder(parent, data, opts) {
    const card = createCard(parent, `Image — ${data.label}`, Object.assign(Object.assign({}, opts), { shadow: true }));
    const imgPad = 16;
...
async function generateSingleSlide(data) {
    switch (data.type) {
        ...
        case 'data-table': return await createDataTableSlide(data);
        default:
